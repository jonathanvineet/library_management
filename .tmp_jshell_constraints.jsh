...
try (var c = java.sql.DriverManager.getConnection(url, user, pwd);
     var ps = c.prepareStatement("select conname, pg_get_constraintdef(oid) as def from pg_constraint where conrelid = 'public.book_requests'::regclass and conname like '%status%'");
