...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('SELECT id, book_id as "bookId", user_id as "userId", status, requested_at as "requestDate", processed_at as "responseDate" FROM book_requests WHERE id = $1', [id]);
